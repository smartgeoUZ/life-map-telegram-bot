...
public class WebHookHandler extends TelegramWebhookBot {
    ...
    public WebHookHandler(Vertx vertx, JsonObject config) {
        super();
    ...
    public SendMessage onWebhookUpdateReceived(Update update) {
        ...
        try {
            if (update.hasCallbackQuery()) {
                sendMessage = handleIncomingCallbackQuery(update.getCallbackQuery());
                return sendMessage;
            }
            ...
            if (update.hasMessage()) {
                ...
                if (message.hasText() || message.hasLocation() || message.hasContact()) {
                    sendMessage = handleIncomingMessage(message);
                }
            }
            ...
            if (update.hasEditedMessage()) {
                ...
                if (editedMessage.hasLocation()) {
                    sendMessage = handleIncomingMessage(editedMessage);
                }
            }
    ...
    private SendMessage handleIncomingMessage(Message message) throws TelegramApiException {
        ...
        return stateProcessing(state, message, language, dbUser);
    }
     ...
     * @return SendMessage
     */
    private SendMessage handleIncomingCallbackQuery(CallbackQuery callbackQuery) {
...
//                }
    ...
    private SendMessage stateProcessing(Integer state, Message message, String language,
                                        JsonObject dbUser) {
        SendMessage sendMessageRequest;
...
//        keyboardFirstRow.add(getShareContactCommand(language));
    ...
    private static String getEventsCommand(String language) {
        return String.format(LocalisationService.getString("events", language),
                Emoji.EVENTS.toString());
    }
